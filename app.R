...
colnames(sars_cases) <- c("country","country_group","date","cases","deaths")

countries = read.csv("input_data/countries_codes_and_coordinates.csv")

ebola_cases = read.csv("input_data/ebola.csv")
colnames(ebola_cases) <- c("country","date","cases","deaths","latitude","longitude","penalty")

h1n1_cases = read.csv("input_data/h1n1.csv")
colnames(h1n1_cases) <- c("region","deaths","projected_deaths","latitude","longitude","penalty")
...
group_cv_cases <- cv_cases %>%
  group_by(country)%>%
  summarize_if(is.numeric, sum, na.rm=TRUE)

group_cv_cases$country <- factor(group_cv_cases$country)
...
#data1$jhu_ID <- factor(data1$jhu_ID)
data1$date <- as.Date(as.character(data1$date), "%Y-%m-%d")

data1 <- data1[, c("country",  "date", "cases","deaths","recovered", "active_cases")]
...
basemap = leaflet(plot_map) %>% 
  ...
  addLayersControl(
    ...
    overlayGroups = c("2019-COVID (new)", "2019-COVID (active)", "2019-COVID (cumulative)", "2003-SARS", "2009-H1N1 (swine flu)", "2014-Ebola"),
    options = layersControlOptions(collapsed = FALSE)) %>% 
  hideGroup(c("2019-COVID (active)", "2019-COVID (cumulative)", "2003-SARS", "2009-H1N1 (swine flu)", "2014-Ebola"))  %>%
...
ui <- bootstrapPage(
  ...
  navbarPage(#theme = shinytheme("flatly"), collapsible = TRUE,
    theme = NULL, collapsible = TRUE,
    "Covid-19 Tracker", id="nav",
    
    
    tabPanel('read.me', icon = icon("book-open"),
             column(3, h3("Welcome to Covid19 Interactive User Interface"),
                    
                    # tags$a( "Copy Rights belong to Unicredit.it", href="https://www.unicredit.it/it/privati.html"),
                    # br(),  
                    tags$b("Please wait/refresh for the map and the app4 to be downloaded.", style="color:red"),
                    h3("Map Tab"),
                    h4("write here some explanations"),
                    h3("Contact"), 
    ...
    tabPanel("Table", icon = icon("table"),
             sidebarLayout(
               sidebarPanel( width = 2, 
                             ...
                             uiOutput("choose_country"),  h6("Select upto 20 countries."),
                             sliderInput("year_range",
                                         label = h4("Range of years: "),
                                         min = min(data$date), # info we already know about our data
                                         max = max(data$date),
                                         value = c(min(data$date), max(data$date))#, # set the starting value
                                         #format = "####"  
                                         #he `format` argument to sliderInput is deprecated. Use `sep`, `pre`, and `post` instead. (Last used in version 0.10.2.2)
               ...
               mainPanel( width = 10,
                          tabsetPanel(type = "tabs", 
                                      tabPanel("Graph", icon = icon("chart-line"),
                                               #plotOutput("ggplot_gdppc_vs_country")#, when you use ggplot not plotly
                                               
                                               
                                               plotlyOutput('ggplot_gdppc_vs_country')
                                      ...
                                      tabPanel("Data",  icon = icon("database"),
                                               ...
                                               br(),
                                               HTML("Filter The Time Series Data Below, then click"),
                                      ...
                                      # tabPanel("Pivot",  rpivotTableOutput("cv.cases.pivot"))
                          )))),
    
    
    
    #  tabPanel('Time-series Data', icon = icon("database"),
    ...
    tabPanel('Data', icon = icon("database"),
             #verbatimTextOutput("Raw2"),
             br(),
             HTML("Filter The Group Data by Country Below, then click"),
             #downloadButton('downLoadFilter2',"Download the filtered data"),
    ...
    tabPanel("About this site", icon = icon("info-circle"),
             tags$div(
               tags$h4("Last update"), 
               h6(paste0(update)),
               ...
               "Code and input data used to generate this Shiny mapping tool are available on ",tags$a(href="https://github.com/eparker12/nCoV_tracker", "nCoV_tracker, "),
               tags$a(href="https://github.com/rstudio/shiny-examples/blob/master/087-crandash/", "crandash, "),
               tags$a(href="https://rviews.rstudio.com/2019/10/09/building-interactive-world-maps-in-shiny/", "building-interactive-world-maps-in-shiny, "),
               tags$a(href="https://github.com/rstudio/shiny-examples/tree/master/063-superzip-example", "063-superzip-example."),
               # 
...
server = function(input, output, session) {
  ...
  country_data <- reactive({
    ...
    subset(data, country %in% input$countries & 
             date %in% c(input$year_range[1]:input$year_range[2])) # get it from the UI! 
  })
  ...
  output$Raw.pivot <- renderPrint({
    #if(input$tabs == 5){
    output$table.pivot <- #DT::renderDataTable(
      # DT::datatable(thedata() ))
      renderTable({ thedata()   })
    #}
  })
  
  output$downLoadFilter.pivot <- downloadHandler(
    filename = function() {
      paste('Filtered data-', Sys.Date(), '.csv', sep = '')
    },
    content = function(file){
      write.csv(thedata()[input[["table_rows_all"]], ],file)
    })
  ...
  # this hold our plot
  output$ggplot_gdppc_vs_country <- renderPlotly ({    #renderPlot({ this is when you arre using ggplot not plotly
    ...
    if(yaxis == "cases"){
      p <- ggplot(country_data(), aes(x = date, y = cases ,color = country))
      ggplotly( ( 
        p + geom_point(size = 1) + 
          theme_minimal() +
          ggtitle("Cases over time")+
          theme(plot.title=element_text(face="bold", size=10))), height = 600
      )
    }
    
    else if(yaxis == "deaths"){
      
      p <- ggplot(country_data(), aes(x = date, y = deaths ,color = country))
      ggplotly(( p + geom_point(size = 1) + 
                   #geom_text_repel(aes(label = country)) + # hjust=1, vjust=1, nudge_x = 1, na.rm = TRUE) +
                   theme_minimal() +
      ...
      p <- ggplot(country_data(), aes(x = date, y = active_cases, color = country))
      ggplotly(( p + geom_point(size = 1) + 
                   
                   #geom_text_repel(aes(label = country)) +# hjust=1, vjust=1, nudge_x = 1, na.rm = TRUE) +
                   theme_minimal() + 
                   ggtitle("Active cases over time")+
                   theme(plot.title=element_text(face="bold", size=10))), height = 600
      )
  ...
  output$dt <- 
    DT::renderDataTable(
      datatable(data1,  options = list(pageLength = 50, scrollX='500px',autoWidth = TRUE),
                filter = "top"),
      server = FALSE
    )
  ...
  output$download_filtered <- 
    downloadHandler(
      ...
      content = function(file){
        write.csv(data1[input[["dt_rows_all"]], ],
                  file)
      }
    )
  ...
  output$dt2 <- 
    DT::renderDataTable(
      datatable(group_cv_cases,  options = list(pageLength = 50, scrollX='500px',autoWidth = TRUE),
  ...
  output$download_filtered2 <- 
    downloadHandler(
      ...
      content = function(file){
        write.csv(group_cv_cases[input[["dt_rows_all"]], ],
                  file)
      }
    )
  ...
  #                 options = list(pageLength = 50, scrollX='500px',autoWidth = TRUE)))
  #}
  #  })
  ...
  thedata2 <- reactive(group_cv_cases)
  output$Raw2 <- renderPrint({
    #if(input$tabs == 5){
    output$table2 <- DT::renderDataTable(
      DT::datatable(thedata2(), filter = 'top',escape = FALSE, 
                    options = list(pageLength = 50, scrollX='500px',autoWidth = TRUE)))
    #}
  })
  ...
  output$downLoadFilter2 <- downloadHandler(
    ...
    content = function(file){
      write.csv(thedata2()[input[["table_rows_all"]], ],file)
    })
